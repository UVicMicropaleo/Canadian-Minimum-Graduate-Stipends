...
GMSm1 <- lmer(scaledGrossStipend ~ 
                   ...
                   + Program
              +Example.Department
                  + (1|University),   data = sos.df)
GMSm1
summary(GMSm1)
visreg(GMSm1, "scaledGrossStipend", by = "endowment")
visreg(GMSm1, "scaledGrossStipend", by = "Program")
visreg(GMSm1, "scaledGrossStipend", by = "Example.Department")
visreg(GMSm1, "scaledGrossStipend")
plot(GMSm1)
AIC(GMSm1)
...
summary(NMSm1)
visreg(NMSm1, "scaledNetStipend", by = "endowment")
visreg(NMSm1, "scaledNetStipend", by = "Program")
visreg(NMSm1, "scaledNetStipend", by = "Example.Department")
visreg(NMSm1, "scaledNetStipend")
plot(NMSm1)
qnorm(NMSm1)
AIC(NMSm1)
...
#chi sq tests for scaled Net Stipend and Endowment
    median(sos.df$endowment,na.rm=T)->ue
    median(sos.df$scaledNetStipend,na.rm=T)->ns
  # or 
 #   1500->ue
  #  15000->ns
abline(v=ue)
...
length(which(sos.df$endowment >= ue & 
               sos.df$scaledGrossStipend >= ns))->up.right
length(which(sos.df$endowment < ue & 
               sos.df$scaledGrossStipend >= ns))->up.left
length(which(sos.df$endowment < ue & 
               sos.df$scaledGrossStipend < ns))->down.left
length(which(sos.df$endowment >= ue & 
               sos.df$scaledGrossStipend < ns))->down.right
# table
quanttable <- as.table(rbind(c(up.left,up.right),c(down.left,down.right)))
quanttable

chisq.test(quanttable)
