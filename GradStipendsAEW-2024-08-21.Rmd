...
#Remove NAs
Gross.Stipend <- sos.df %>%
  filter(!is.na(Gross.Stipend))

#Histogram of values - visually inspect to see if skewed
hist1 <- hist(sos.df$Gross.Stipend,main = "Gross Stipend",xlab = "Value ($CAD)")

#Scale stipend 
sos.df$scaledGrossStipend <- scale(sos.df$Gross.Stipend, center = TRUE, scale = TRUE)
med.stipend <- median(sos.df$scaledGrossStipend, na.rm = TRUE)
med.stipend

#Histogram of values - visually inspect to see if skewed
hist2 <- hist(sos.df$scaledGrossStipend,main = "Gross Stipend",xlab = "Value ($CAD)")
mean.scaledGrossStipend <- (mean(sos.df$scaledGrossStipend, na.rm = TRUE))
mean.scaledGrossStipend
```
